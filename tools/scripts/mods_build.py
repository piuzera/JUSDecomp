...
For each modded NitroFS file the tool:
  1. locates the ORIGINAL in the stock ROM by content search
     (originals come from extract/files/<same path>),
  2. matches the FAT entry that starts exactly there,
  3. checks growth against the slack before the next FAT extent,
  4. emits `[[mods.overlays]]` entries: the data overlay plus, for grown
     files, an overlay of the FAT entry (start/end) so the game sees the new
     size. The ROM itself is never touched.
...
def find_original(rom: bytes, relpath: str) -> int:
...
def resolve_mod_files(rom: bytes, mods_dir: Path) -> tuple[list[tuple[int, str]], list[str]]:
    ...
    for modfile in sorted(mods_dir.rglob("*")):
        ...
        if len(parts) >= 3 and parts[1].endswith(".aar"):
            ...
            add_data(abs_off, modfile,
                     ...
                     f"({size} bytes)")
            continue
        # ---- plain NitroFS file (growth supported) ----
        orig_off = find_original(rom, rel)
        fid = None
        ...
        if growth > 0 and (gap is None or growth > gap):
            raise SystemExit(
                f"[!] {rel}: growth {growth} exceeds slack {gap}")
